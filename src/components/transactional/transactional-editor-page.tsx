...
interface TransactionalEditorPageProps {
  campaignId: string;
}

/* --- Main Component --- */

export function TransactionalEditorPage({ campaignId }: TransactionalEditorPageProps) {
  const router = useRouter();
  const [campaign, setCampaign] = useState<TransactionalData | null>(null);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!campaignRes.ok) {
          router.push("/transactional");
          return;
        }

        const campaignData: TransactionalData = await campaignRes.json();
          ...
          {editingName && !isSent ? (
            <input
              ...
              onBlur={() => {
                ...
                if (!name.trim()) setName("Untitled email");
              }}
              onKeyDown={(e) => {
                ...
                <div className="space-y-2">
                  ...
                  {selectedList && (
                    <p className="text-[12px] text-muted-foreground">
                      {selectedList.contactCount.toLocaleString()} contacts will receive this email
                    </p>
                  )}
                </div>
              )}
          ...
          {!isSent && (
            <div className="mb-4">
              ...
              <div className="rounded-lg border border-border bg-background p-4">
                ...
                {sendMode === "now" ? (
                  <p className="text-[12px] text-muted-foreground">
                    Email will be sent immediately when you click &quot;Send now&quot;.
                  </p>
                ) : (
                  <div className="space-y-3">
